 ...
 * Date: 10/25/18, 21:26
 **/
import {HashTable} from "../src/open-addressing/HashTable";


describe("HashTable with Open addressing", () => {
    ...
    it("should generate hashes for the keys", () => {
        ...
        expect(myTable.hash(994)).toBe(327);
        expect(myTable.hash("994")).toBe(327);


        expect(myTable.hash("abc")).toBe(590);
        expect(myTable.hash("acb")).toBe(589);
        expect(myTable.hash("bca")).toBe(587);
        expect(myTable.hash("cba")).toBe(586);
        expect(myTable.hash("cab")).toBe(587);
